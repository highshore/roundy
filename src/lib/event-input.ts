...
export function eventInput(body:unknown) {
 ...
 return {...(v.venue_description===undefined?{}:{venue_description:text('venue_description',1000,false)}),nationality_requirements:validateRequirements(v.nationality_requirements),title:text('title',120),description:text('description',3000),starts_at,duration_minutes,lockdown_minutes,reminder_minutes,venue:text('venue',160),address:text('address',300,false),latitude:latitude as number|null,longitude:longitude as number|null,age_min,age_max,capacity,status,images:images as string[],image:images[0]||'',neighborhood:'Seoul',theme:category,ends_at:new Date(Date.parse(starts_at)+duration_minutes*60000).toISOString()};
}
