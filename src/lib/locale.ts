...
const koreanUi: Record<string, string> = {
  ...
  'SEOUL / AFTER HOURS':'서울 / 일과 후 만남', 'A real room. A fresh start. ':'실제 만남, 새로운 시작. ',
  ...
  'Create a reusable profile and get verified once. Then reserve an available event directly with a valid ticket or payment.': '프로필 등록과 인증은 최초 1회만. 이후에는 티켓이나 결제로 원하는 이벤트를 바로 신청하세요.',
  'Bring ID and arrive early. Meet through 15-minute English rounds. No alcohol is provided. Entry closes 15 minutes after the start.': '신분증을 지참하고 여유 있게 도착해 주세요. 15분씩 영어로 대화를 나눠요. 주류는 제공되지 않으며, 시작 15분 후에는 입장이 제한돼요.',
  'Choose up to 3 Yes. Both say Yes? You see each other’s profile, name and phone number after the event.': '최대 3명과 매칭을 시도할 수 있어요. 두 사람 모두 Yes를 선택하면 이벤트 후 서로의 프로필, 이름과 전화번호를 확인 가능해요.',
