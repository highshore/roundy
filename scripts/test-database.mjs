...
await denied(1,`select wis_redeem('${eid}')`,/cancellation guidelines/);
...
const booking=(await as(1,'select wis_redeem($1,true) id',[eid])).rows[0].id;
assert.equal((await as(1,'select wis_redeem($1,true) id',[eid])).rows[0].id,booking);
...
await as(6,"insert into wis_events(id,title,starts_at,duration_minutes,venue,address,capacity,age_min,age_max,status) values($1,'Referral event',now()+interval '2 days',90,'Venue','Address',12,18,100,'live')",[referralEvent]);
const referralCode=(await as(1,'select wis_get_or_create_referral_code() code')).rows[0].code;
assert.match(referralCode,/^[A-Z0-9]{6}$/);
const selfQuote=(await as(1,'select wis_referral_quote($1,1) q',[referralCode])).rows[0].q;
assert.equal(selfQuote.valid,false);assert.equal(selfQuote.reason,'self');
const friendQuote=(await as(2,'select wis_referral_quote($1,1) q',[referralCode])).rows[0].q;
...
const referralResult=(await as(2,'select wis_redeem_referral($1,1,$2,true) r',[referralEvent,referralCode])).rows[0].r;
assert.ok(referralResult.booking_id);assert.equal(referralResult.final_amount,0);
assert.equal((await db.query('select remaining from wis_credit_lots where id=$1',[referralResult.credit_lot_id])).rows[0].remaining,0,'Free one-ticket referral is consumed by the booking');
await denied(2,'select * from wis_referral_codes',/permission denied/);
await denied(2,`select wis_redeem_referral('${referralEvent}',1,'${referralCode}',true)`,/already been used/);
