...
-- A ticket is the enrolment entitlement. Profile approval, consent and the
-- event's gender split are all enforced here so API clients cannot bypass them.
...
begin
  ...
  if coalesce(p_terms_accepted,false) is not true then raise exception 'Confirm the cancellation guidelines and terms before enrolling';end if;
  ...
  if now()>=e.starts_at-make_interval(mins=>e.lockdown_minutes) then raise exception 'Event registration is locked';end if;
  select profile into profile_data from public.wis_profiles where user_id=u;
  if profile_data is null or coalesce(profile_data->>'full_name','')='' or coalesce(profile_data->>'birth_date','')='' or coalesce(profile_data->>'gender','') not in ('male','female') or coalesce(profile_data->>'nationality','')='' or coalesce(profile_data->>'job_title','')='' or coalesce(profile_data->>'workplace','')='' or coalesce((profile_data->>'height_cm')::int,0) not between 100 and 250 or coalesce(profile_data->>'phone','')!~'^010-[0-9]{4}-[0-9]{4}$' or coalesce((profile_data->>'contact_consent')::boolean,false)=false or coalesce(jsonb_array_length(profile_data->'photos'),0) not between 1 and 3 or coalesce(jsonb_array_length(profile_data->'interests'),0) not between 3 and 10 then raise exception 'Complete all required profile fields before enrolling';end if;
  select status into verification_status from public.wis_verifications where user_id=u;
  if coalesce(verification_status,'')<>'Verified' then raise exception 'Your profile must be approved before enrolling';end if;
  ...
  if e.theme='1:1 Speed Meetup' then
    ...
    from public.wis_bookings b join public.wis_profiles p on p.user_id=b.user_id
    where b.event_id=e.id and p.profile->>'gender'=member_gender;
    if same_gender_bookings>=e.capacity/2 then
      raise exception 'The % seats for this event are currently full',case member_gender when 'female' then 'women''s' else 'men''s' end;
    end if;
  end if;
  ...
  insert into public.wis_bookings(event_id,user_id,credit_lot_id,terms_accepted_at) values(p_event,u,credit,now()) returning id into booking;
  return booking;
end;
$$;

-- Retire the former one-argument path; callers must record explicit consent.
...
revoke all on function wis_private.redeem(uuid),wis_private.redeem(uuid,boolean),public.wis_redeem(uuid),public.wis_redeem(uuid,boolean) from public,anon;
