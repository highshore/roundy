...
Deno.serve(async req=>{
 ...
 const {data:jobs,error}=await db.rpc('wis_claim_reminders');if(error)return response({error:'Could not claim reminder jobs'},500);
 let sent=0,failed=0;
 for(const job of jobs||[]){
  const phone=String(job.phone||'').replace(/^\+82/,'0').replace(/\D/g,'');
  if(!/^01[016789]\d{7,8}$/.test(phone)){await db.from('wis_reminder_deliveries').update({status:'failed',last_error:'A Korean mobile number is required for this Alimtalk sender',updated_at:new Date().toISOString()}).eq('id',job.id);failed++;continue;}
  try{
   ...
   const {error:saveError}=await db.from('wis_reminder_deliveries').update({status:'sent',provider_reference:String(data.message?.requestId||job.id),updated_at:new Date().toISOString()}).eq('id',job.id);
   ...
   await db.from('wis_reminder_deliveries').update({status:'failed',last_error:'Delivery not confirmed. Reconcile the provider request before retrying.',updated_at:new Date().toISOString()}).eq('id',job.id);failed++;
  }
 }
